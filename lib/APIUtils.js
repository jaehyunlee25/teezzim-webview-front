...
const OUTER_ADDR_HEADER = 'https://dev.mnemosyne.co.kr:1009';
...
export const sendResponse = parameter => {
  const { command, param } = JSON.parse(parameter);
  if (command === 'getClub') {
    log(param);
    post(
    ...
    window.BRIDGE.sendResponse('hello, android!!');
  }
  if (command === 'test2') {
